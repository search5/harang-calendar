...
- **`{{hrcal:` 단계별 자동완성** — `{{hrcal:`을 입력하면 계정 → 캘린더 → (날짜 또는 이벤트 제목) 순서로 자동완성이 이어집니다. `{{hrcal:<계정id>:<캘린더이름>:date:yyyy-mm-dd}}`(그 날짜의 이벤트 목록이 카드로 펼쳐지는 위젯) 또는 `{{hrcal:<계정id>:<캘린더이름>:event:<uid>}}`(인라인 칩)이 삽입됩니다. 칩(또는 날짜 위젯 안의 카드)을 클릭하면 상세 팝업을 볼 수 있습니다. 팝업에서 계정을 고를 때는 여전히 눈에 보이는 계정 이름으로 검색합니다 — `<계정id>`는 계정의 내부 id로, 자동으로 삽입될 뿐 직접 입력하는 일은 없습니다. 이름이 아니라 id이기 때문에, 나중에 계정 이름을 바꿔도 이미 삽입해 둔 참조는 깨지지 않습니다. `<캘린더이름>`은 여전히 캘린더의 표시 이름이므로, 캘린더 이름을 바꾸면 이전과 마찬가지로 기존 참조가 깨질 수 있습니다.
...
- **오래된 참조 경고** — `{{hrcal:...}}` 참조가 가리키는 계정 또는 캘린더가 더 이상 설정된 항목과 일치하지 않으면(예: 캘린더 이름을 바꿨거나, 계정이 더는 존재하지 않는 경우) 날짜 위젯에 경고를 표시합니다.
